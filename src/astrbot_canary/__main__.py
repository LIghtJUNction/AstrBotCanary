...
from astrbot_canary.base import AstrbotModuleAPI
# 安装错误堆栈追踪器
rich.traceback.install()
logger = logging.getLogger("astrbot_canary.loader")

AstrbotModuleAPI.Utils.set_logging_basic_config(logging.INFO)
...
def main(LoaderName: str = "canary_loader", LoaderArgs: list[str] = ["astrbot.modules.core.canary_eventbus","astrbot.modules.web.canary_web"]):
    ...
    AstrbotLoaderCls: EntryPoint | None = AstrbotModuleAPI.Utils.load_from_entrypoint("astrbot.modules.loader", LoaderName)
    ...
    AstrbotLoader: AstrbotModuleAPI.AstrbotBaseModule = AstrbotLoaderCls.load()()
...
if __name__ == "__main__":
    ...
    _loaderArgs = sys.argv[2:] if len(sys.argv) > 2 else ["astrbot.modules.web.canary_web"]
