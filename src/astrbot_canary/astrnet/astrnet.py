...
class AstrbotRequests:
    ...
    def _send_request(
        ...
        if cls._network is None:
            raise RuntimeError(
                "AstrbotRequests network not set. Call set_network first.",
            )
...
if __name__ == "__main__":
    ...
    async def hello_handler(data: Hello) -> Response[HelloResponse]:
        return Response(
            body=HelloResponse(
                message=f"Hello, Astrbot! foo={data.foo}, bar={data.bar}",
            ),
            status_code=200,
        )
    ...
    async def status_handler(request: Request) -> Response[StatusResponse]:
        return Response(
            body=StatusResponse(status="OK", query_params=request.query_params),
            status_code=200,
        )
