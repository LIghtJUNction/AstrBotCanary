...
## 最近改动
- 开发期间我把pre-commit几乎全关了
后续稳定后再开启

- 目前状态是无法运行的，我改了很多很多代码没有同步更改

- 比如我将db.py整个移除了，然后依赖注入库我整个进行升级。并且原本写在api库内的两个实验性的模块装饰器。
- 我移除了，这样确保api库的干净整洁
- 确保api库只能包含协议，ABC抽象基类，枚举类，还有异常类
...
- 生命周期函数，C#风格（具体来说就是Unity的Monobehaviour类的三个生命周期）
