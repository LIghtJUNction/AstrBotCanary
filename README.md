...
- 宏观上，astrbot_api（偏向于基础设施）
此包内几乎只包含枚举类，以及抽象协议类
被所有模块/插件直接依赖
但存在高级封装：比如模块定义装饰器，简单的依赖注入装饰器（不支持第二种实现方式）

- 微观上
astrbot_canary：
包含根模块，核心模块。
core包单向导出，根模块首先从核心内获取到配置类，统一路径获取类（基于astrbot root的规范路径），数据库类（基于sqlite数据库）
然后注册到api模块的依赖注入器类中保存

接下来才开始正常启动流程（稍后介绍）

astrbot_canary_api：
最底层的类是enum类，其次是interface
最后是装饰器类，均为单向依赖关系。
...
根模块Awake,然后Start
从配置文件中读取预配置的模块启动列表
直接依据配置，加载对应入口点指向的模块类

由pluggy库控制核心模块首先启动，web模块/tui模块最后启动。

- 如果不从配置启动，则尝试发现模块，发现模块后对模块进行分类，分为三类：

加载器模块/核心模块/UI模块
每一类最多加载一个模块


其中，除了正式进入异步循环之前，均为同步。
